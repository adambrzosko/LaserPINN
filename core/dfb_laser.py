...
def make_laser(cavity_type='dfb', **kwargs):
    """Create a laser parameter set for the specified cavity type.

    Parameters
    ----------
    cavity_type : str
        'dfb' — DFB with HR-coated rear facet (R2=0.95, default)
        'fp'  — Fabry-Perot with cleaved facets (R1=R2=0.32)
    **kwargs
        Override any DFBLaserParams field.

    Returns
    -------
    DFBLaserParams
        Derived quantities (alpha_m, tau_p, etc.) are recomputed
        automatically from the chosen reflectivities.

    Examples
    --------
    >>> dfb = make_laser('dfb')                # default DFB
    >>> fp  = make_laser('fp')                 # FP with R1=R2=0.32
    >>> fp2 = make_laser('fp', L=500e-6)       # longer FP cavity
    """
    if cavity_type == 'fp':
        fp_defaults = dict(R2=0.32, cavity_type='fp')
        fp_defaults.update(kwargs)
        return DFBLaserParams(**fp_defaults)
    elif cavity_type == 'dfb':
        kwargs.setdefault('cavity_type', 'dfb')
        return DFBLaserParams(**kwargs)
    else:
        raise ValueError(f"Unknown cavity_type: {cavity_type!r}. Use 'dfb' or 'fp'.")
...
if __name__ == '__main__':
    ...
    # Show FP comparison
    fp = make_laser('fp')
    fp_Ith = fp.threshold_current()
    frac_dfb = (1 - params.R1) / ((1 - params.R1) + (1 - params.R2))
    frac_fp = (1 - fp.R1) / ((1 - fp.R1) + (1 - fp.R2))
    print(f"Fabry-Perot comparison (R1=R2=0.32):")
    print(f"  Mirror loss:       {fp.alpha_m:.0f} m⁻¹ (vs DFB {params.alpha_m:.0f} m⁻¹)")
    print(f"  Photon lifetime:   {fp.tau_p*1e12:.2f} ps (vs DFB {params.tau_p*1e12:.2f} ps)")
    print(f"  Threshold current: {fp_Ith*1e3:.1f} mA (vs DFB {I_th*1e3:.1f} mA)")
    print(f"  Front facet frac:  {frac_fp:.2f} (vs DFB {frac_dfb:.2f})")
    print()
