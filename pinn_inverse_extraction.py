...
from laser_pinn import (
    LaserPINN, LearnableParams, Scales,
...
# ── Generate gain-switched pulse train ──────────────────────────────────────

def generate_gain_switched_data(params, f_rep=5e9, n_periods=10,
                                 pts_per_period=400):
    """
    Generate a gain-switched pulse train at f_rep using solve_transient.

    Returns dict with time, N, S, phi arrays covering n_periods.
    """
    I_th = params.threshold_current()
    I_off = 0.9 * I_th   # below threshold
    I_on = 5.0 * I_th    # well above threshold
    T_rep = 1.0 / f_rep
    t_on = 0.3 * T_rep   # 30% duty cycle
    ...
    def I_func(t):
        ...
        if t_in_period < t_rise:
            f = 0.5 * (1.0 - np.cos(np.pi * t_in_period / t_rise))
            return I_off + (I_on - I_off) * f
        elif t_in_period < t_on - t_rise:
            return I_on
        elif t_in_period < t_on:
            f = 0.5 * (1.0 - np.cos(np.pi * (t_on - t_in_period) / t_rise))
            return I_off + (I_on - I_off) * f
        else:
            return I_off
...
def add_noise_snr(S_clean, snr_db, seed=42):
    """Add Gaussian noise to intensity trace at specified SNR (dB)."""
    rng = np.random.default_rng(seed)
    S_power = np.mean(S_clean**2)
    noise_power = S_power / (10.0 ** (snr_db / 10.0))
    noise = rng.normal(0, np.sqrt(noise_power), size=S_clean.shape)
    S_noisy = S_clean + noise
    S_noisy = np.maximum(S_noisy, 1e-10)  # keep physical
    return S_noisy
...
if __name__ == '__main__':
    ...
    fig1.savefig(os.path.join(OUT_DIR, 'parameter_convergence.png'), dpi=150,
                 bbox_inches='tight')
    plt.close(fig1)
    print(f"  Saved: images/pinn_inverse/parameter_convergence.png")

    # Plot 2: Fit quality
    fig2 = plot_fit_quality(model_25, data_25, scales_25, params_true, gs_data)
    fig2.savefig(os.path.join(OUT_DIR, 'fit_quality.png'), dpi=150,
                 bbox_inches='tight')
    plt.close(fig2)
    print(f"  Saved: images/pinn_inverse/fit_quality.png")
    ...
    fig3.savefig(os.path.join(OUT_DIR, 'noise_robustness.png'), dpi=150,
                 bbox_inches='tight')
    plt.close(fig3)
    print(f"\n  Saved: images/pinn_inverse/noise_robustness.png")
