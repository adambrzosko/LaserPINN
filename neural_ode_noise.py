...
from dfb_laser import DFBLaserParams, q
from million_pulse_comparison import simulate_pulses


# ── Output directory ─────────────────────────────────────────────────────────

OUT_DIR = 'images/neural_ode'
...
gt_dphi = np.angle(np.exp(1j * np.diff(gt_phi)))

gt_mean_S = np.mean(gt_peak_S)
gt_std_S = np.std(gt_peak_S)
gt_r1 = np.abs(np.mean(np.exp(1j * gt_dphi)))
...
for b in range(nsde_peak_phi.shape[1]):
    ...
    dphi = np.angle(np.exp(1j * np.diff(phi_seq)))
...
plt.tight_layout()
fig1.savefig(os.path.join(OUT_DIR, 'noise_amplitudes.png'), dpi=150)
plt.close(fig1)
print(f"\n  Saved: {OUT_DIR}/noise_amplitudes.png")
...
plt.tight_layout()
fig2.savefig(os.path.join(OUT_DIR, 'statistics_comparison.png'), dpi=150)
plt.close(fig2)
print(f"  Saved: {OUT_DIR}/statistics_comparison.png")
...
plt.tight_layout()
fig3.savefig(os.path.join(OUT_DIR, 'training_loss.png'), dpi=150)
plt.close(fig3)
