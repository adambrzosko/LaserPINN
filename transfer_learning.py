...
from laser_pinn import (
    LaserPINN, Scales, LearnableParams, generate_reference,
    make_collocation, physics_residuals, compute_loss, subsample,
    DTYPE, device,
)

# ── Output directory ─────────────────────────────────────────────────────────
os.makedirs('images/transfer_learning', exist_ok=True)


# ── Helpers ──────────────────────────────────────────────────────────────────

def add_noise_snr(tensor, snr_db=25, seed=None):
    ...
    return tensor + noise_std * torch.randn_like(tensor)
...
if __name__ == '__main__':
    ...
    plot_finetuning_comparison(
        ...
        scales_sim, 'images/transfer_learning/finetuning_comparison.png'
    )
