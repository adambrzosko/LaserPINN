...
from gsdfb.pinn_utils import (
    Scales, LearnableParams, generate_reference, make_collocation, subsample,
    DEVICE as device, DTYPE,
)
from gsdfb.plotting import setup_plotting, save_fig, img_dir

# ── Output directory ─────────────────────────────────────────────────────────
OUT_DIR = img_dir('bayesian_pinn')
...
if __name__ == '__main__':
    ...
    plot_posterior_distributions(
        ...
        os.path.join(OUT_DIR, 'posterior_distributions.png')
    )

    plot_predictive_uncertainty(
        predictions, ref, scales, params,
        os.path.join(OUT_DIR, 'predictive_uncertainty.png')
    )

    plot_training_convergence(
        history,
        os.path.join(OUT_DIR, 'training_convergence.png')
    )
