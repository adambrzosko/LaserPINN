...
def mode_setup(laser, n_modes=N_MODES):
    ...
    # DFB grating enhancement: main mode has extra feedback
    # Side modes see ~3-10 dB less effective reflectivity
    # Model as additional loss for side modes: lower effective tau_p
    # Equivalently, reduce gain factor to account for higher threshold
    # For a well-designed DFB, SMSR > 30 dB in CW → gain penalty ~ 1-5%
    # Under gain switching, this small difference gets amplified by the
    # exponential gain dynamics.
    grating_penalty = np.ones(n_modes)
    for m in range(1, n_modes):
        # Side modes see higher mirror loss (weaker grating feedback)
        # Typical: 2-5 cm^-1 extra loss for nearest side modes
        order = (m + 1) // 2
        extra_loss = 300.0 * order   # m^-1 extra mirror loss
        # Express as fraction of threshold gain
...
if __name__ == '__main__':
    ...
    for f_rep in freqs:
        ...
        for mode_label, S_inj_val in [('free', 0.0), ('sld', 3e19)]:
            ...
            # Phase correlation
            dphi_m = np.angle(np.exp(1j * np.diff(phi_m)))
            dphi_s = np.angle(np.exp(1j * np.diff(pk_phi_s)))
            r1_multi = float(np.abs(np.mean(np.exp(1j * dphi_m))))
            r1_single = float(np.abs(np.mean(np.exp(1j * dphi_s))))
